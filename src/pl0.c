...
// readPL0Tokens() is defined in pl0-lexer.c, which is generated by flex from
// the lex file "pl0-vector.l".

int pl0GrammarGenerated = 0;   // Whether or not getPL0Grammar has already been called.
struct grammar pl0Grammar;

struct grammar getPL0Grammar() {
    // Return the cached result if getPL0Grammar has already been called.
    if (pl0GrammarGenerated)
        return pl0Grammar;
    ...
    // Cache the result so we don't regenerate it every time.
    pl0Grammar = grammar;

    return grammar;
}

struct parseTree parsePL0Tokens(struct vector *lexemes) {
    return parse(lexemes, getPL0Grammar(), "program");
}
